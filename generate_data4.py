...
def toggle_promotions(user_data, include_promotions):
    if include_promotions:
        products = user_data["carrito"]["productos"]
        user_data["carrito"]["promociones"] = [generate_promotion(product["SKU"]) for product in products]
    else:
        user_data["carrito"]["promociones"] = []

if __name__ == "__main__":
    # Generar 10 usuarios con promociones
    users_with_promotions = generate_multiple_users(2500, include_promotions=True)
    
    # Generar 10 usuarios sin promociones
    users_without_promotions = generate_multiple_users(2500, include_promotions=False)
    
    # Combinar los datos
    data_array = users_with_promotions + users_without_promotions

    # Calcular el 40% de los datos para repetir
    num_to_repeat = ceil(0.50 * len(data_array))

    # Seleccionar y duplicar el 40% de los datos
    repeated_data = random.sample(data_array, num_to_repeat)
    
    # Dividir los datos repetidos en dos mitades
    half_num_to_repeat = ceil(num_to_repeat / 2)
    
    # Cambiar promociones en la primera mitad
    for user in repeated_data[:half_num_to_repeat]:
        toggle_promotions(user, include_promotions=True)
    
    # No cambiar promociones en la segunda mitad
    for user in repeated_data[half_num_to_repeat:]:
        toggle_promotions(user, include_promotions=False)
    ...
    print("Datos generados, mezclados, con modificaciones de promociones, y guardados en 'fakeData.json'.")